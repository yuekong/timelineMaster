...
public class VideoTimeBar extends ScalableScaleBar implements ScalableScaleBar.TickMarkStrategy {
    ...
    float tickValueBoundOffsetH = 10;
    ...
    void init() {
        tickValueBoundOffsetH = SizeUtils.dp2px(getContext(), 1);
...
//            Log.e(TAG, " same mode.");
        ...
        switch (m) {
            ...
            case MODE_UINT_30_MIN:
                this.mMode = m;
                updateScaleInfo(1 * 60 * 60 * 1000, 5 * 60 * 1000);
...
//        Log.e("TAG", " mode: " + mMode);
    ...
    protected void onEndTickDraw(Canvas canvas) {
        ...
        float startY = getBaselinePosition() + 1.0f-getHeight();
    ...
    protected void drawCursor(Canvas canvas, float cursorPosition, long cursorValue) {
        ...
        // 倒三角形顶边的 y
        float topSidePosition = statY - mTriangleHeight - 50;
        path.moveTo(startX, statY);
        path.lineTo(startX - 3.5f-3, topSidePosition);
        path.lineTo(startX + 3.5f+3, topSidePosition);
        path.close();
